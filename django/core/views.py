...
def render(req, url, extra={}):
    ...
    extra.update(
        entries_options=util.list_entries(),
...
def index(request):
    ...
    if request.method == "POST":
        ...
        # Handles A-Z sorted list in index main
            ...
            entry_list = list(filter(lambda x: x.lower().startswith(letter), entry_list))
...
def wiki_entry(request, title):
    ...
    return render(request, "core/base_entry.html", context)
...
def saveHandler(request, **kwargs):
    ...
    if content and title:
        title = title.strip()
        util.save_entry(title.strip(), str(content).strip())
...
def create_update(request, title=""):
    ...
    if request.method == "POST":
        ...
        if submit is None:
            ...
            messages.warn(request, " You must add a title and content to create entry!")
            return render(request, "core/create_edit_entry.html", context)
...
def random_entry(request):
    ...
    messages.error(request, "Opp... Something went wrong!")
    return redirect(index)
...
def delete_entry(request, title, deletion=None):
    ...
    context["title"] = title
    return render(request, "core/delete_entry.html", context)
